...
function NavigationMenu() {
    ...
    const [checkedBrands, setCheckedBrands] = useState({}); // Track checked state for each brand
    const [allSelected, setAllSelected] = useState(false); // Track state for "All Brands"
    ...
    const handleAllBrandsClick = () => {
        setAllSelected(true); // Mark "All Brands" as selected
        setCheckedBrands({}); // Deselect all individual brands
    };

    // Handle individual brand click
    const handleBrandClick = (brand) => {
        setAllSelected(false); // Deselect "All Brands"
        ...
        <nav className="flex items-center p-4 shadow-lg">
            ...
            <div className="flex items-center justify-between w-full">
                <ul className="flex flex-wrap">
                    ...
                    <li className="mr-4 flex items-center">
                        <span
                            className={`cursor-pointer text-sm font-medium ${
                                allSelected ? 'border border-purple-500 bg-purple-100 rounded px-2' : 'text-black'
                            }`}
                    ...
                    {/* Individual Brands (Display only top 5 brands) */}
                    {brands.length > 0 ? (
                        brands.slice(0, 5).map((brand, index) => (
                            <li className="mr-4 flex items-center" key={index}>
                                <span
                                    className={`cursor-pointer text-sm font-medium ${
                                        checkedBrands[brand]
                                            ? `${brandStyles[index]} border px-2 rounded` // Apply styles for selected brands
                                            : 'text-black'
                                    }`}
